...
# up letters and digits in this dictionary.
    ...
    # If the symbol is a key in the dictionary, the correct symbol will be returned. Otherwise, the
    def handle_symbol_end(self):
        try:
            x = self._morse_codes[self.current_symbol]
            self.current_symbol = ""
            return x
        except KeyError:
    ...
    #If there exists a letter, the letter will be added to current word
    def update_current_word(self, letter):
        print(letter)
        if letter != None:
            self.current_word += letter

    #If there exists a word, the word will be printed, and then reset
    ...
    def main(self):
        ...
        m = mocoder()
        while True:
            signal = self.read_one_signal(self.serial_port)
            print(signal)
            if signal[0] == 48:
                m.update_current_symbol(signal)
            elif signal[0] == 49:
                m.update_current_symbol(signal)
            elif signal[0] == 50:
                m.update_current_word(m.handle_symbol_end())
            elif signal[0] == 51:
                m.handle_word_end()
            print(m.current_word)
